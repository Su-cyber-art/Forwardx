...
ForwardX 大多数设置都可以在后台页面中配置。环境变量主要用于本地 systemd 服务启动、数据库连接、登录密钥、Telegram Token 和升级命令这类“启动前就要确定”的配置。

本地 systemd 部署时，环境变量通常写在安装目录的 `.env` 文件中，并由 systemd 服务加载。

::: warning 修改后要重启
环境变量只会在面板进程启动时读取。修改 `.env` 或 systemd 环境配置后，需要重启 `forwardx-panel.service`。
:::
...
`JWT_SECRET` 变化后，已登录用户会需要重新登录。建议首次部署时就在 `.env` 中固定一个随机值，后续升级不要更换。
:::
...
| `MYSQL_HOST` | 空 | MySQL 地址。不要把面板本机回环地址误当远程数据库地址。 |
...
| 场景 | 推荐填写 |
| --- | --- |
| 数据库在面板同一台服务器 | `127.0.0.1` 或服务器内网 IP，取决于数据库监听地址 |
| 数据库在另一台服务器 | 面板服务器能访问到的内网 IP、公网 IP 或域名 |
...
::: tip
AI API Key 不建议写入 `.env`。后台保存后会做脱敏展示，也方便管理员切换供应商和模型。
:::
...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `FORWARDX_TARGET_VERSION` | 空 | 本地部署可指定安装或升级到某个版本，例如 `v2.3.188`。 |
...
普通用户通常不需要配置这些变量。只有在 GitHub 访问不稳定、内网分发或排查 Agent 升级问题时才会用到。
