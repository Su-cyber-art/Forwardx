...
- 教程站点新增环境变量说明页，补充基础运行、数据库、连接池、Telegram、AI 助手、升级、Agent 安装脚本和调试日志变量说明。
- 部署教程继续保留当前 fork 的本地 systemd 部署口径。
...
- 修复 UDP over TCP 运行时在 GitHub Release 构建时缺少 arm64 资产导致面板包和 Agent 包无法完整发布的问题。
...
- 新增 VitePress 用户教程内容，文档保留在仓库内供本地查看和维护。
