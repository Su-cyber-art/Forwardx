...
[![Version](https://img.shields.io/badge/version-2.3.186-brightgreen.svg)](https://github.com/Su-cyber-art/Forwardx/releases)
[![Node.js](https://img.shields.io/badge/Node.js-22+-339933?logo=node.js&logoColor=white)](https://nodejs.org/)

## 当前 Fork 说明

本仓库是基于原项目 [poouo/Forwardx](https://github.com/poouo/Forwardx) fork 后维护的个人二次修改版。

- 当前仓库：[Su-cyber-art/Forwardx](https://github.com/Su-cyber-art/Forwardx)
- 原始上游：[poouo/Forwardx](https://github.com/poouo/Forwardx)
- 本仓库保留原项目的 AGPL-3.0-only 开源协议与主要功能基础。
- 本仓库中的 README、安装脚本示例和 Release 下载默认指向当前个人 fork。
- 当前个人 fork 明确只保留本地二进制包 + systemd 部署方式。
- 本仓库会按个人需求进行二次修改，例如界面展示、部署文案、脚本路径和发布入口可能与上游不完全一致。
- 本仓库不是原项目官方发布渠道；如果需要原版，请以原始上游仓库说明为准。

## 仓库与下载

- 当前仓库：https://github.com/Su-cyber-art/Forwardx
- 在线教程：https://su-cyber-art.github.io/Forwardx/
- GitHub Releases：https://github.com/Su-cyber-art/Forwardx/releases
- Android APK 最新下载：https://github.com/Su-cyber-art/Forwardx/releases/download/v2.3.186/forwardx-android-v2.3.57.apk
...
- 原地升级会保留已有配置和数据；本地 systemd 部署会保留安装目录下的 `data` 目录。
...
这些参数通过环境变量调整，MySQL 和 PostgreSQL 共用同一组连接池变量；SQLite 不使用连接池参数。调整时要同时确认数据库服务端的连接上限：PostgreSQL 的 `max_connections`、MySQL 的 `max_connections` 应大于所有 ForwardX 实例 `DATABASE_MAX_OPEN_CONNS` 的总和，并预留运维、备份和其他程序连接余量。本地 systemd 部署可在服务环境变量或安装目录环境配置中增加变量后重启 `forwardx-panel.service`。
...
下面的命令会下载当前 fork 的 Release 二进制包，安装运行依赖，写入 systemd 服务并启动面板。
...
| `FORWARDX_TARGET_VERSION` | 空 | 本地二进制部署可指定安装或升级版本，例如 `v2.3.12` |
...
下载地址：
https://github.com/Su-cyber-art/Forwardx/releases/download/v2.3.186/forwardx-android-v2.3.57.apk
...
[![Star History Chart](https://api.star-history.com/svg?repos=Su-cyber-art/Forwardx&type=Date)](https://www.star-history.com/#Su-cyber-art/Forwardx&Date)
